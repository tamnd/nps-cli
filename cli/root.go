// Package cli assembles the nps command tree from the nps
...
// NewApp assembles the kit application from the nps domain. The
...
//
// To add a command, declare it in nps/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := nps.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(nps.Domain{}).Register(app)
