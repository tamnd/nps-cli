...
describes what users get. Pick a format with `-o`, or let nps choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nps <command> -o table   # aligned columns for reading
nps <command> -o jsonl   # one JSON object per line, for piping
nps <command> -o json    # a single JSON array
nps <command> -o csv     # spreadsheet friendly
nps <command> -o tsv     # tab-separated
nps <command> -o url     # just the URL column
nps <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nps <command> --fields id,title,url
```
...
```bash
nps <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nps <command>            # a table, because this is a terminal
nps <command> | wc -l    # JSONL, because this is a pipe
```
